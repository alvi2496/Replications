...
for k in freq.values():
    ...
    if project not in map_proj_design_general:
        ...
        if category == "design":
            map_proj_design_general[project] = (map_proj_design_general[project][0] + 1,
                                                map_proj_design_general[project][1])
        else:
            map_proj_design_general[project] = (map_proj_design_general[project][0],
                                                map_proj_design_general[project][1] + 1)


print("Project,Design,General,Proportion")
for k, v in map_proj_design_general.items():
    csv_line = k+',' + str(v[0]) + ',' + str(v[1]) + ',' + str(float(v[0]) / (v[0] + v[1]))
